...
- 상세한 이관 내역은 [`UPSTREAM.md`](UPSTREAM.md)를, 원본 기술 가이드는 [`docs/UPSTREAM-README.md`](docs/UPSTREAM-README.md)를 참조하십시오.
...
└── docs/               화면 정의서 · 요구사항 실측표 · 프로젝트 기술 문서
```
...
> **디자인 원칙**: 전문 연구원의 장시간 데이터 판독 피로도를 최소화하기 위해 **모노크롬(Monochrome) 기반의 고대비 미니멀 UI**를 적용했습니다. 상태 인디케이터(5종)에만 정밀하게 정의된 강조 색상을 사용하여 이상 징후를 즉각 식별할 수 있으며, 흑백 출력물이나 화면 캡처 보고서에서도 정보 손실이 발생하지 않습니다. (상세 규격: [`docs/화면정의서.md`](docs/화면정의서.md))
...
## 6. 기능 구현 현황 및 로드맵 (Implementation Status)

실측 기반의 투명한 구현 현황을 제공합니다. 상세 측정 근거 및 요구사항 추적표는 [`docs/요구사항-동작실측표.md`](docs/요구사항-동작실측표.md)를 참고하십시오.

| 상태 | 범주 | 세부 구현 내용 |
|:---:|:---:|---|
| **구현 완료**<br>*(Supported)* | **코어 엔진 및 웹 콘솔** | • 노드 기반 DAG 설계 · 검증 · 위상 정렬 스케줄러<br>• 모델 레지스트리 및 연산 라우팅 모듈<br>• 분산 임대(Lease) 기반 작업 큐 및 자동 장애 복구<br>• Run 단위 데이터 적재, 이력 관리, 1:1 대조 분석<br>• 파이프라인 재실행(Fork) 및 레거시 데이터 마이그레이션 도구<br>• 핵심 6대 관리 콘솔 화면 구현 |
| **실증 진행 중**<br>*(In Progress)* | **엔드포인트 연동** | • **실 GPU 연산 엔드포인트 실증**<br>  - RunPod · HTTP 엔드포인트 · 로컬 컨테이너 어댑터 구현 완료<br>  - 현재 시험용 모의(Mock) 어댑터 기반 정합성 검증 완료 후 실 GPU 파이프라인 연계 중 |
| **개발 로드맵**<br>*(Roadmap)* | **고도화 기능** | • 전사 통합 관제 대시보드<br>• 3D 단백질 입체 구조 인터랙티브 뷰어 (pLDDT/RMSD 기반)<br>• 자연어 프롬프트 기반 워크플로우 설계 코파일럿(Copilot)<br>• 조직 및 프로젝트 단위 세부 역할 기반 접근 제어(RBAC) |
